...
>[!AVAILABILITY]
>
>Esse recurso está com disponibilidade limitada. Entre em contato com seu representante da Adobe para obter acesso.
...
* [Fragmentos](../content/fragments.md) não são compatíveis entre os modos Usar Temas e Estilo Manual.

   * Fragmentos com temas não estão disponíveis em conteúdos de email criados sem o uso de temas.

   * Para usar um [fragmento](../content/fragments.md) em um conteúdo com tema, este fragmento deve ter sido criado por si mesmo, usando temas. [Saiba mais](#leverage-themes-fragment)
...
1. Para começar, crie um novo [modelo de conteúdo](../content/create-email-templates.md).

1. Selecione a opção **[!UICONTROL Criar ou editar temas]**.

   ![Editor de modelo de conteúdo mostrando a opção Criar ou editar temas](assets/theme-create.png)

1. Selecione um tema do Adobe. Neste exemplo, selecione o **[!UICONTROL Tema padrão]** e clique em **[!UICONTROL Criar]**.

   ![Seletor de temas com tema padrão selecionado e botão Criar](assets/theme-select.png)
...
1. Você também pode selecionar um modelo personalizado na guia **[!UICONTROL Meus temas]** e clicar em **[!UICONTROL Editar]** para atualizá-lo.
   ...
   ![Guia Meus temas listando temas personalizados com a opção Editar destacada](assets/theme-edit.png)

1. Na guia **[!UICONTROL Configurações gerais]**, comece a definir seu tema dando a ele um nome específico que corresponda à sua marca. Você pode ajustar a largura padrão do visor para seus emails<!--and also export the current theme for reuse-->.
...
1. Use o painel à direita para navegar pelas diferentes guias e atualizar as configurações de design.
   ...
   ![Painel direito do editor de temas com guias para cores, texto, espaçamento e outras configurações de design](assets/theme-right-pane.png)
...
1. Na guia **[!UICONTROL Cores]**:
   ...
   * Use o botão **[!UICONTROL Editar]** para configurar uma **[!UICONTROL paleta de cores]** com cores padrão para sua marca. Selecione uma **[!UICONTROL Predefinição]** para criar rapidamente um esquema de cores ou ajustar cada cor do seu tema individualmente. Você também pode usar uma combinação de ambos.
     ...
     ![Animação de edição da paleta de cores do tema com predefinições e cores personalizadas](assets/theme-colors.gif)
   ...
   * Clique em **[!UICONTROL Adicionar variante]** para criar várias variantes de cores, como o modo claro e escuro, em que cada variante do tema tem sua própria paleta de cores e controles de nuance.
     ...
     ![Variantes de cores no editor de temas com Adicionar variante para paletas adicionais](assets/theme-colors-variant.png)
   ...
   * Para cada variante, clique no ícone **[!UICONTROL Editar]** para editar qualquer elemento individual. É possível usar a paleta padrão criada ou qualquer cor personalizada.
     ...
     ![Animação de edição de cores individuais em uma variante de cores de tema](assets/theme-colors-edit-variant.gif)
...
1. Nas **[!UICONTROL Configurações de texto]**, é possível definir a fonte global que deseja usar para todo o tema. Para um controle mais granular, também é possível editar cada texto de cabeçalho e parágrafo para ajustar a fonte, o tamanho, o estilo e assim por diante.
   ...
   ![Guia Configurações de texto para estilos globais de fonte e cabeçalho ou parágrafo em um tema](assets/theme-text.png)

   >[!NOTE]
   >
   >Ao selecionar fontes personalizadas da Web, observe que muitos clientes de email, como Gmail e Yahoo! não suportam fontes externas da web e recorrerão às fontes do sistema. Considere incluir fontes substitutas para garantir que o conteúdo seja exibido corretamente em todos os clientes de email. [Saiba mais](#themes-guardrails)
...
1. Usando as outras guias à direita, você pode gerenciar separadamente cada elemento de botão, divisor, formatação de imagem adicional e espaçamento do layout de grade para esse tema.
   ...
   ![Controles de estilo de botão no painel direito do editor de temas](assets/theme-buttons.png)
...
1. Em [!DNL Adobe Campaign], [crie um email de entrega](create-email.md) ou trabalhe com base em uma entrega existente, ou crie um email [modelo de conteúdo](../content/create-email-templates.md#create-template-from-scratch) e [edite o conteúdo do email](get-started-email-designer.md#start-authoring).

1. Você pode selecionar uma das seguintes ações:

   * Selecione um [modelo de email](../content/use-email-templates.md) interno para abrir o Designer de email. Um tema padrão específico para cada modelo é aplicado automaticamente.

   * Crie um [novo conteúdo do zero](create-email-content.md) e selecione **[!UICONTROL Usar Temas]** para começar com um tema de estilo predefinido.

     ![Tela inicial do Email Designer com as opções Usar Temas e Estilo Manual](assets/theme-from-scratch.png)
...
1. No Designer de Email, clique no botão **[!UICONTROL Temas]** no painel direito. O tema padrão ou o tema do modelo é exibido. Você pode alternar entre as duas variantes de cor para esse tema.
   ...
   ![Tela Email Designer com o painel Temas aberto mostrando o tema ativo e as variantes de cores](assets/theme-default-hero.png)

1. Clique na seta ao lado do tema usado no momento. A lista de temas personalizados e do Adobe disponíveis é exibida.

   ![A lista suspensa de Temas foi expandida para mostrar temas do Adobe e Meus temas](assets/theme-hero-change.png)

1. Clique em **[!UICONTROL Meus temas]** e selecione um tema criado.

   ![Lista de meus temas com um tema criado pelo usuário selecionado no seletor de temas](assets/theme-select-custom.png)
...
1. Quando um tema for selecionado em um modelo de conteúdo, você poderá clicar no botão **[!UICONTROL Editar tema]** para atualizá-lo. [Saiba mais](#create-and-edit-themes)
   ...
   ![Modelo de conteúdo no Email Designer com o botão Editar tema no painel Temas](assets/theme-edit-in-template.png){width="40%"}
...
1. Se você aproveitar um tema usando várias variantes de cores, poderá escolher uma variante específica para um determinado componente de estrutura. Isso permite definir uma variante de cor para todo o conteúdo e usar uma variante diferente para apenas uma estrutura específica.
   ...
   ![Estrutura selecionada com a opção Usar variante de tema específica na guia Estilos](assets/theme-structure-variant.png)
...
![Guia Estilos em um componente selecionado mostrando o ícone de desbloquear estilo](assets/theme-unlock-style.png){width="90%"}
...
![Componente de texto desbloqueado com cor de texto vermelha personalizada em um plano de fundo branco](assets/theme-unlock-style-ex-white.png){width="80%" align="center" zoomable="yes"}
...
![Mesmo texto desbloqueado com cor vermelha retida e plano de fundo atualizado de um novo tema](assets/theme-unlock-style-ex-colored.png){width="80%"}
...
1. No [!DNL Adobe Campaign], crie um fragmento visual e clique em **[!UICONTROL Criar]** para criar o conteúdo do fragmento. [Saiba como](../content/create-fragment.md#create-from-scratch)

1. Selecione **[!UICONTROL Usar Temas]** para começar com um tema de estilo predefinido.

   ![Opções de início do Fragment Email Designer com a opção Usar temas selecionada](assets/fragment-use-themes.png){width="100%"}
...
1. Clique no botão **[!UICONTROL Temas]** no painel direito. O tema padrão é exibido. Você pode alternar entre as diferentes variantes de cor para esse tema.
   ...
   ![O conteúdo do fragmento no painel Email Designer com Temas mostra o tema padrão](assets/fragment-default-theme.png){width="100%" align="center" zoomable="yes"}
...
1. É possível selecionar outros temas para visualizar o conteúdo do fragmento. Para fazer isso, selecione a seta ao lado do tema padrão e clique em **[!UICONTROL Selecionar temas]**.
   ...
   ![Cabeçalho do tema com seta e controle Selecionar temas para visualização do fragmento](assets/fragment-select-themes.png){width="40%"}
...
1. Você pode navegar entre as guias **[!UICONTROL Temas do Adobe]** e **[!UICONTROL Meus temas]** e selecionar até cinco temas compatíveis (de ambas as guias) para o fragmento.
   ...
   ![Caixa de diálogo Selecionar temas compatíveis com temas do Adobe e guias Meus temas](assets/fragment-select-compatible-themes.png){width=70%}
...
1. Selecione novamente a seta ao lado do **[!UICONTROL Tema padrão]**. Agora é possível alternar entre os diferentes temas selecionados para visualizar cada renderização de estilo.
   ...
   ![Tela de fragmento com vários temas selecionados para visualização no painel Temas](assets/fragment-selected-themes.png){width=90%}
...
[!DNL Adobe Campaign] permite converter um modelo que foi criado usando o estilo manual em um conteúdo compatível com tema. Isso pode ser particularmente útil se você criou modelos de conteúdo antes de os temas serem introduzidos no [!DNL Adobe Campaign], ou se você estiver importando conteúdo externo.
...
1. Abra um [modelo de conteúdo](../content/create-email-templates.md) do email e edite o conteúdo usando o Designer de Email.

1. Selecione o ícone **[!UICONTROL Temas]** no painel direito e clique no botão **[!UICONTROL Gerar tema do conteúdo]**.

   ![Painel de temas com o botão Gerar tema a partir do conteúdo realçado](assets/generate-theme.png){width=100%}

1. A janela **[!UICONTROL Criar um tema]** é aberta. O [!DNL Adobe Campaign] detecta automaticamente os elementos de estilo e os consolida em um novo tema.

   ![Criar uma caixa de diálogo de tema que resume os estilos detectados do conteúdo do modelo](assets/generate-theme-create-window.png){width=90%}
