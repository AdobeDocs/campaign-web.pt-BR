...
Os scripts executados no contexto de um fluxo de trabalho acessam uma série de **objetos** globais adicionais, como o próprio fluxo de trabalho que está sendo executado (`ìnstance`), suas várias tarefas (`task`) ou os eventos que ativaram determinada tarefa (`event`).
